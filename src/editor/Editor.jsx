...
const DEFAULT_TEXT = {
	legacy:
		'The assignment contents are not available for editing. Some features may be disabled.',
};

const t = scoped('assignment.editing', DEFAULT_TEXT);

function getPartComponentForAssignment(assignment) {
	return assignment && assignment.isDiscussion
...
export default class AssignmentEditor extends React.Component {
	...
	render() {
		...
		const cls = cx('assignment-editor', {
			loading: !!assignment,
			'read-only': readOnly,
		});
		...
		return (
			<StickyContainer className={cls}>
				...
				{readOnly ? (
					...
					<div>
						{assignment && !assignment.isModifiable && (
							<Panels.MessageBar message={t('legacy')} />
						)}
						<AssignmentInfo

...
const logger = Logger.get(
	'lib:assignment-editor:controls:components:PreviewControls'
);
...
export default class PreviewControls extends React.Component {
	...
	handleClick = () => {
		if (this.props.previewAssignment) {
			...
			logger.warn(
				'Preview button does not link to assignment... Doing no-op...'
			);
		}
	};
		...
		return (
			<div className="assignment-preview-controls">
				<div className="preview-trigger" onClick={this.handleClick}>
					...
					<span className="preview-trigger-text">Preview</span>
				</div>
			</div>
		);
	}

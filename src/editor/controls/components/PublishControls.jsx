...
class PublishControls extends React.Component {
	...
	onChange = value => {
		...
		const state =
			PublishStateMap[value] ||
			(value instanceof Date ? value : void value);

		return allowPublish(assignment, state).then(
			() => {
				return assignment
	...
	hasPublishingLinks() {
		...
		return (
			assignment &&
			(assignment.hasLink('publish') || assignment.hasLink('unpublish'))
		);
	}
	...
	render() {
		...
		const value = Publish.evaluatePublishStateFor({
			isPublished: () =>
				...
				assignment.getPublishDate() < Date.now(),
			getPublishDate: () => assignment && assignment.getPublishDate(),
		});
		const cls = cx('assignment-publish-controls', {
			'is-saving': isSaving,
		});

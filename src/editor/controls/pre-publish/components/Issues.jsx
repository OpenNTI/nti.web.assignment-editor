...
const DEFAULT_TEXT = {
	title: 'Are you sure?',
	errorMessage:
		'One or more questions could not be saved and will revert to a previous version.',
...
const t = scoped(
	'assignment.editing.controls.publish.confirmation',
	DEFAULT_TEXT
);
...
export default class PrepublishModal extends React.Component {
			...
			<div className="assignment-confirm-publish">
				<div className={cls}>
					...
					<div className="message">
						...
						{warningMessage && (
							<div className="content"> {warningMessage}</div>
						)}
						{this.renderExpand()}
					</div>
	...
	renderExpand() {
		...
		return text && collapsed ? (
			<div className="expand" onClick={this.doExpand}>
				{text}
			</div>
		) : null;
	}
	...
	renderIssues() {
		...
		const cls = cx('issues', {
			collapsed,
			'has-both': hasErrors && hasWarnings,
		});
		const sort = x =>
			orderBy(
				x,
				v =>
					v &&
					v.attachedTo &&
					(v.attachedTo.label || '').toLowerCase()
			);
		...
		return (
			<div className={cls} onClick={this.onIssueClick}>
				{hasErrors && hasWarnings && (
					<div className="header error">{t('errorHeader')}</div>
				)}
				{hasErrors && (
					<ErrorList errors={errors} onErrorFocus={this.onCancel} />
				)}
				{hasErrors && hasWarnings && (
					<div className="header warning">{t('warningHeader')}</div>
				)}
				{hasWarnings && (
					<ErrorList
						errors={warnings}
						isWarnings
						onErrorFocus={this.onCancel}
					/>
				)}
			</div>
		);

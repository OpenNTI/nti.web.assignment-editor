...
const DEFAULT_TEXT = {
	...
	nonInstructorDesc: 'The instructor must reset this assignment before a publish change can occur.'
};
			...
			<div className="inline-reset-menu">
				<div className="reset">
					<div className="nti-checkbox">
						<span className="publish-reset-label">{nonInstructorAdmin ? t('nonInstructorLabel') : t('resetLabel')}</span>
						<span className="publish-reset-text">{nonInstructorAdmin ? t('nonInstructorDesc') : t('resetDesc')}</span>
						{!nonInstructorAdmin && <div className="publish-reset" onClick={this.props.onReset}>{t('resetAssignment')}</div>}
					</div>
				</div>
			</div>
		);

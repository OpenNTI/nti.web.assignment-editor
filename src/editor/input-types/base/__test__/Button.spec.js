...
const render = (node, cmp, props, ...children) =>
	new Promise(
		next =>
...
describe('Assignment Sidebar Button Tests', () => {
				...
				question_set: {
					...
					questions: [
						...
						},
						{
							...
							MimeType: 'application/vnd.nextthought.naquestion',
							content:
								"Which of the following were used by southerners to deprive African Americans of their constitutional rights during the Reconstruction Era? <b class='bfseries'>Select all that apply.</b>",
							parts: [
								{
									...
									solutions: [
										{
											Class:
												'MultipleChoiceMultipleAnswerSolution',
											MimeType:
						...
						},
						{
							...
							MimeType: 'application/vnd.nextthought.naquestion',
							content:
								'5. Students can mark up images and illustrate concepts to share using the ____________.',
							parts: [
								{
	...
	assignment.getQuestions = () =>
		assignment.parts[0].question_set.questions.map(x => {
	...
	test('tests creation of Button', done => {
		testRender({ assignment: assignment })
			.then(cmps => cmps.forEach(button => expect(button).toBeTruthy()))
			.then(done, done.fail);
	});
	...
	test('tests count of essay questions', done => {
		...
		testRender({ assignment: assignment, handles: handles })
			.then(cmps =>
				cmps.forEach(button => expect(button.getUsedCount()).toBe(1))
			)
			.then(done, done.fail);
	});

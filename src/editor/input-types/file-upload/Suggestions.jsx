...
export default class Suggestions extends React.Component {
	...
	getSuggestions() {
		...
		const suggestions = tokens.map(token => {
			const target =
				Object.values(TYPE_GROUPS).find(x => x.includes(token)) || [];
			return target.filter(
				x => !tokens.includes(x) && !dismissed.includes(x)
			);
		});

		return suggestions.reduce(
			(acc, list) => acc.concat(list.filter(x => !acc.includes(x))),
			[]
		);
	}
			...
			<div>
				{list.length > 0 && (
					<div className="input-type-fileupload-suggestions-wrapper">
						<div className="title">
							People who entered these extensions also added:
						</div>

...
class Buffer extends React.Component {
	...
	nullOrValue(submissionBuffer) {
		return submissionBuffer === false || submissionBuffer === null
			? null
			: submissionBuffer;
	}
	...
	save = () => {
		...
		if (assignment.submissionBuffer !== submissionBuffer) {
			assignment
				.setSubmissionBuffer(this.nullOrValue(submissionBuffer))
				.catch(error => {
					const newSubmissionBuffer = this.nullOrValue(
						assignment.submissionBuffer
					);

					this.setState({
						submissionBuffer: newSubmissionBuffer,
						error,
					});
				});
		}
	...
	timeChanged = value => {
		...
		if (value === 0 && this.state.bufferPolicy === ALLOW_BUFFER_TIME) {
			...
			this.setState({
				submissionBuffer: value,
				error: 'Buffer time must be non-zero',
			});
		} else {
			this.setState(
				{ submissionBuffer: value, error: null },
	...
	setOpenBuffer = () => {
		this.setState(
			{
				bufferPolicy: OPEN_SUBMISSIONS,
				submissionBuffer: null,
				error: null,
			},
			this.saveWithTimeout
		);
	...
	setStrictBuffer = () => {
		this.setState(
			{ bufferPolicy: STRICT_LIMIT, submissionBuffer: 0, error: null },
			this.saveWithTimeout
		);
	};
	...
	renderPolicySelect() {
		const options = [OPEN_SUBMISSIONS, ALLOW_BUFFER_TIME, STRICT_LIMIT];

		return (
			<SelectBox
				options={options}
				onChange={this.onPolicyChange}
				value={this.state.bufferPolicy && this.state.bufferPolicy.value}
				showSelectedOption
			/>
		);
	}

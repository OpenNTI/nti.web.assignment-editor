...
export function deleteQuestionFrom(question, questionSet, assignment) {
	...
	if (orderedContents.canEdit && orderedContents.length === 1) {
		removePartWithQuestionSet(assignment, questionSet).then(methods => {
			if (methods.undo) {
			...
			.then(undo => {
				...
				if (undo) {
					dispatch(UNDO_CREATED, {
						...
						name: 'Undo',
						onComplete: () => {
							const newQuestionSet = getQuestionSetFrom(
								questionSet.NTIID,
								assignment
							);

							if (newQuestionSet) {
								orderedContents.updateBackingObject(
									newQuestionSet
								);
								undo();
							}
...
export function duplicateQuestionFrom(question, questionSet, delaySave) {
	...
	wait(10).then(() => {
		...
		orderedContents
			.insertAt(clone, index + 1, delaySave)
			.catch(maybeResetAssignmentOnError(questionSet))
			.then(() => {
				dispatch(QUESTION_SET_UPDATED, questionSet);
				dispatch(SAVE_ENDED);
...
export function detachSharedQuestion(question, questionSet, delaySave) {
	...
	orderedContents
		.replaceItem(question, clone, delaySave)
		.catch(maybeResetAssignmentOnError(questionSet))
		.then(() => {
			dispatch(QUESTION_SET_UPDATED, questionSet);
			dispatch(SAVE_ENDED);

...
const t = scoped('nti-assignment.navigation-bar.status.AssignmentStatus', {
	...
	passingScore:
		'<span class="percentage">%(passingScore)s%%</span> <span>to Pass</span>',
	attempts: {
		started: {
			limitedAttempts: {
		...
		<div className="time-limit">
			...
			<span>
				{DateTime.getShortNaturalDuration(
					assignment.getMaximumTimeAllowed()
				)}
			</span>
		</div>
	);
...
function AssignmentPassingScore(assignment, historyItem) {
	...
	const failed =
		...
		!assignment.CompletedItem.Success;
...
export default class AssignmentStatus extends React.Component {
	...
	render() {
		...
		const parts = PARTS.map(fn => fn(assignment, historyItem)).filter(
			part => !!part
		);
		...
		return (
			<ul
				className={cx('assignment-navigation-bar-status-assignment', {
					'has-submission': !!historyItem,
				})}
			>
				{parts.map((part, key) => {
					return <li key={key}>{part}</li>;
				})}
			</ul>
		);
	}
